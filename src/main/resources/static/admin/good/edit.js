...
$(() => {
    ...
    // 数据回填
    form.val("edit-form", editGood);
    // 回填分类名称
    $("[name=categoryName]").val(editGood.category.name);
    //  回填品牌名称
    $("[name=brandName]").val(editGood.brand.name);
    // 回填单图片
    let url_single = ctx + "/static/" + editGood.pic;
    fillSingleImage(url_single);

    // 回填多图片
    let other_url = editGood.otherPics;
