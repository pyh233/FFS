...
$(() => {
    ...
    upload.render({
        ...
        before: function (obj) {
            ...
            if (resp.success) {
                ...
                $(".logo-preview").css("background-image", "url(" + url + ")");

                sessionStorage.setItem("logo_url", resp.data);
            } else {
                layer.msg("上传图片失败");
            }
        },
    ...
    // 数据回填
    form.val("edit-form", editBrand);
    if (editBrand.logo && editBrand.logo !== '') {
        let url = ctx + "/static/" + editBrand.logo;
        $(".logo-preview").css("background-image", "url(" + url + ")");
    }
})
...
function doEdit(cb) {
    ...
    // 获取logo数据
    let logo_url = sessionStorage.getItem("logo_url")
    if (logo_url) {
        formData.logo = logo_url;
        sessionStorage.removeItem("logo_url");
    }
    formData.id = editBrand.id;
